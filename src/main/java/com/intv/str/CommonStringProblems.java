...
public class CommonStringProblems {
  ...
  /**
   * find all substrings for a string. Example : for String "abc" all possible substrings are : [a,
   * ab, abc, b, bc, c] O(n^2)
   *
   * @param process
  ...
  /**
   * Given a string, check if it is palindrome O(n)
   *
   * @param input
  ...
  /**
   * Find the longest prefix and suffix from a string. Example: abcabc -> abc | abcabca-> abca |
   * aaaa-> aaa The solution is to build a window n, run between 0 to (length - n) and 1 to lenght
   * match character from the beginning, break if any character does not match for the loop and
   * increase n by 1 The order of comparison should go like this 0 -> length-1 and 1 to length 0 ->
   * length-2 and 2 to length 0 -> length-3 and 3 to length 0 -> length-4 and 4 to length and so on
   * till the boundary conditions are true. O(n)
   *
   * @param input
  ...
  public String longestCommonPrefixAndSuffix(final String input) {
    ...
    for (int i = 0; i < input.length() - subWindow; subWindow++) {
      ...
      if (input.charAt(i) == input.charAt(subWindow)) {
        for (int j = i, k = subWindow;
            k < input.length() && j < input.length() - subWindow;
            j++, k++) {
